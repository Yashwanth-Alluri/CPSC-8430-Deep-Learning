...
optimizer = AdamW(qa_model.parameters(), lr=2e-5, weight_decay=0.01)
        ...
        for para in section['paragraphs']:
            ...
            for qa_item in para['qas']:
                ...
                for answer in qa_item['answers']:
                    ...
                    answers.append({
                        'text': answer['text'].lower(),'start': answer['answer_start'],'end': answer['answer_start'] + len(answer['text'])
                    })
...
class QADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        enc.update({'start_positions': torch.tensor(self.labels[idx]['start']),'end_positions': torch.tensor(self.labels[idx]['end'])
        })
        return enc
...
def train_epoch(model, data_loader, optimizer):
    ...
    for batch in tqdm(data_loader, desc="Training"):
        optimizer.zero_grad()
        outputs = model(input_ids=batch['input_ids'].to(device), attention_mask=batch['attention_mask'].to(device),token_type_ids=batch['token_type_ids'].to(device),start_positions=batch['start_positions'].to(device),end_positions=batch['end_positions'].to(device))
        loss = outputs.loss
...
for epoch in range(EPOCHS):
    train_loss = train_epoch(qa_model, train_loader, optimizer)
